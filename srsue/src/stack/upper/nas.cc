...
void nas::start_tracking_area_update_request(srsran::establishment_cause_t cause_)
{
  ...
  srsran::console("Tracking Area Update Request with cause %s.\n", to_string(cause_).c_str());
  if (state.get_state() != emm_state_t::state_t::registered) {
    logger.info("NAS in invalid state for Tracking Area Update Request");
    logger.info("Tracking Area Update request ignored. State = %s", state.get_full_state_text().c_str());
    return;
  }
  ...
  gen_tracking_area_update_request(msg);
  if (not rrc->connection_request(cause_, std::move(msg))) {
    logger.error("Error starting RRC connection");
    return;
  }
...
void nas::gen_tracking_area_update_request(srsran::unique_byte_buffer_t& msg)
{
  if (msg == nullptr) {
